...
num_reps = 10;
phase_levels = 0.1:10/num_reps:10;
phase_levels = datasample(phase_levels,num_reps,'replace',false)
pattern_names = {'static_test_yaw_90.mat'
                 'static_test_yaw_270.mat'
};

fixation_pattern = 1;
[num_starfield_patterns,n] = size(pattern_names)
starfield_pattern = 2;
...
%Panel_com('set_AO',[3 int16(one_volt*-1)])
%Panel_com('set_AO',[4 int16(one_volt*0.1)]) %phase level
Panel_com('start');
pause(20);
...
for rep = 1:num_reps
    ...
    for i = datasample([1:num_starfield_patterns,;ones(1,num_starfield_patterns)],num_starfield_patterns,2,'replace',false)
        ...
        %Panel_com('set_AO',[3 int16(one_volt*-1)])
        %Panel_com('set_AO',[4 int16(one_volt*phase_level)]) %phase level
        Panel_com('start');
        pause(5);

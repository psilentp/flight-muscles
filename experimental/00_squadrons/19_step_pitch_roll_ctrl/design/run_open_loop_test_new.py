...
vel_range = [1]
dir_range = [('pth_roll_%s'%p,1+9*i*1/14.) for i,p in enumerate(range(0,360,30))]
rep_range = range(4)
...
pattern_data = dict()
count = 2 #pattern_names[0] with a pattern_index of 1 is fixation
...
for rep in rep_range:
    pattern_data['step_%s_v%s_rep%s'%('multipole',vel,rep)] = dict()
    pattern_data['step_%s_v%s_rep%s'%('multipole',vel,rep)]['condition_tuple'] = ('multipole',
                                                                                vel,
                                                                                rep)
    pattern_data['step_%s_v%s_rep%s'%('multipole',vel,rep)]['pattern_index'] = count
    pattern_data['step_%s_v%s_rep%s'%('multipole',vel,rep)]['condition_voltage'] = 1+9*12*1/14.
    count += 1

for rep in rep_range:
    pattern_data['step_%s_v%s_rep%s'%('phsrndm',vel,rep)] = dict()
    pattern_data['step_%s_v%s_rep%s'%('phsrndm',vel,rep)]['condition_tuple'] = ('phsrndm',
                                                                                vel,
                                                                                rep)
    pattern_data['step_%s_v%s_rep%s'%('phsrndm',vel,rep)]['pattern_index'] = count
    pattern_data['step_%s_v%s_rep%s'%('phsrndm',vel,rep)]['condition_voltage'] = 1+9*13*1/14.
    count += 1

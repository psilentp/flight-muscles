...
for epoch0_duration in [0,0.250,0.50,1.0]:
	...
	io.savemat('./stimulus_data/position_function_pos_%ss.mat'%(epoch0_duration),{'func':func})
...
for epoch0_duration in [0,0.250,0.50,1.0]:
	if epoch0_duration > 0:
		...
		func = temp_freq_ramp(**params)
	func += np.abs(np.min(func)) + 1
	io.savemat('./stimulus_data/position_function_neg_%ss.mat'%(epoch0_duration),{'func':func})
	plb.plot(func)
##########################

...
def SrvGetVersionResponse(**kwargs):
    print kwargs
...
class LEDPanels():
    ...
    def panel_com(self,*args):
        command = PanelCommand()
        command.command = args[0]
        for i,arg in enumerate(args[1:]):
            #print arg
            command.__dict__['arg%s'%str(i+1)] = arg
        self.PanelsCommand_callback(command)
        time.sleep(0.01)
    ...
    def __init__(self):
        ...
        self.serialport = 'COM3' # self.DiscoverSerialPort() 
        if (self.serialport is not None):
            try:
                ...
                print('ledpanels serial port could not be opened:' % e)
        else:
            print('ledpanels serial port was not specified as a ROS parameter, nor was it found automatically.')
    ...
    def MakeSurePortIsOpen(self):
        while not self.serial.isOpen():
            try:
                print('ledpanels: Trying to open serial port %s ...' % self.serialport)
                self.serial.open()
            except serial.SerialException:
                print('s')
                time.sleep(1)
            else:
                print('ledpanels: Opened serial port %s' % self.serialport)
                ...
                if (nBytesArg>1):
                    ...
                    if (args_dict_list[iArg]['unsigned']==False):
                        ...
                        if (command == 'set_ao'):
                            ...
                            print('ledpanels: command argument type (signed int) not yet implemented.')
        else:
            print('Unknown ledpanels command: %s' % command)
    ...
    def Signed_byte_to_chr(self, byte):
        #print chr((256+byte) % 256)
        return chr(int((256+byte) % 256))
    ...
    def Dec2chr(self, num, nChars):
        ...
        char_list = [0] * nChars
        if (num > 2**(8*nChars)):
            print('Dec2chr() overflow: Not enough characters for a number of this size.')
        if (num < 0 ):
            print('Dec2chr() out of range: This function does not handle negative numbers.' );
